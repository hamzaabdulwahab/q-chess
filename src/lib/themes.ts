// Board theme data + helpers. This module is framework-agnostic (NOT a client
// component) so it can be imported from both the client ThemeProvider and from
// Server Components (e.g. the root layout, which reads the theme cookie and
// emits the per-theme CSS variables before paint).
...
export const GLOBAL_THEME_KEY = "chess-theme-global";
export const GLOBAL_THEME_USER_SET_KEY = "chess-theme-global-user-set";

export function isValidThemeId(themeId: string | null | undefined): boolean {
  return Boolean(themeId && chessThemes.some((theme) => theme.id === themeId));
}

// Always resolves to a real theme. Unknown/corrupted ids fall back to Forest
// Green (the product default), NOT to chessThemes[0] (which is Dark Night).
export function getThemeById(themeId: string | null | undefined): ChessTheme {
  return (
    chessThemes.find((t) => t.id === themeId) ??
    chessThemes.find((t) => t.id === DEFAULT_THEME_ID) ??
    chessThemes[0]
  );
}

// Emits one `[data-chess-theme="<id>"] { --sq-light: ...; ... }` block per theme.
// Injected once (server-side) so the board paints correct colors from the
// pre-paint data attribute via CSS variables, with no React render involved.
export function themeVarsCss(): string {
  return chessThemes
    .map(
      (t) =>
        `[data-chess-theme="${t.id}"]{` +
        `--sq-light:${t.lightSquare};` +
        `--sq-dark:${t.darkSquare};` +
        `--sq-light-hover:${t.lightHover};` +
        `--sq-dark-hover:${t.darkHover};` +
        `--coord-on-light:${t.lightCoord};` +
        `--coord-on-dark:${t.darkCoord};` +
        `--board-border:${t.borderColor};` +
        `--last-move:${t.lastMoveHighlight};` +
        `--move-hint:${t.moveHighlight};` +
        `--check-highlight:${t.checkHighlight};` +
        `}`,
    )
    .join("");
}
